// ==UserScript==
// @name         Gemini Counter Ultimate (v7.2-debug)
...
(function () {
    ...
    const GLOBAL_KEYS = {
        ...
        MODULES: 'gemini_enabled_modules'  // 新增：模块开关存储
    };
    ...
    const CounterModule = {
        ...
        init() {
            ...
            this.bindEvents();
            console.log('💎 CounterModule initialized');
        },

        destroy() {
            // 清理事件监听器 (实际上很难完全清理，这里只是占位)
            console.log('💎 CounterModule destroyed');
        },
    ...
    const PanelUI = {
        ...
        update() {
            ...
            if (acctBadge) {
                ...
                acctBadge.textContent = acctLabels[cm.accountType];
                acctBadge.style.background = acctColors[cm.accountType];
                acctBadge.style.color = acctTextColors[cm.accountType];
            }
    ...
    function debugDumpStorageKeys() {
        try {
            ...
            console.warn('💎 Debug: failed to list storage keys', e);
        }
    }
    ...
    function debugShowDetectedUser() {
        try {
            ...
            console.warn('💎 Debug: failed to show detected user', e);
        }
    }
    ...
    function debugExportAllStorage() {
        try {
            ...
            console.warn('💎 Debug: failed to export storage', e);
        }
    }
    ...
    function debugExportLegacyData() {
        try {
            ...
            console.warn('💎 Debug: failed to export legacy data', e);
        }
    }
    ...
    function debugDumpGeminiStores() {
        try {
            ...
            console.warn('💎 Debug: failed to dump storage', e);
        }
    }

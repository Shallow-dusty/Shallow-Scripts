// ==UserScript==
// @name         Gemini Counter Ultimate (v7.4-debug)
...
(function () {
    ...
    console.log("💎 Gemini Assistant v7.4-debug (Modular - Pure Enhancement) Starting...");
    ...
    const Logger = (() => {
        ...
        const MAX_STORE = 500;
        let level = GM_getValue(GLOBAL_KEYS.LOG_LEVEL, 'info');
        const buffer = Array.isArray(GM_getValue(GLOBAL_KEYS.LOGS, []))
            ? GM_getValue(GLOBAL_KEYS.LOGS, [])
            : [];
        ...
        function persist() {
            ...
            saveTimer = setTimeout(() => {
                const trimmed = buffer.slice(-MAX_STORE);
                GM_setValue(GLOBAL_KEYS.LOGS, trimmed);
            }, 200);
        }
    ...
    const PanelUI = {
        ...
        openSettingsModal() {
            ...
            version.textContent = 'Gemini Assistant v7.4-debug (Modular)';
        ...
        openDebugModal() {
            ...
            const renderLogs = () => {
                ...
                let entries = Logger.getEntries().slice(-120);
                if (activeFilter !== 'all') {
                ...
                if (searchTerm) {
                    entries = entries.filter(e => {
                        ...
                        return `${e.msg} ${dataStr}`.toLowerCase().includes(searchTerm);
                    });
                }
                if (entries.length === 0) {
